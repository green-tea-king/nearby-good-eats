...
- `交通` 模式採同心圓擴張搜尋：走路依序查 800m / 1200m / 1600m / 2000m / 3000m / 5000m，開車依序查 3000m / 5000m / 8000m / 12000m / 20000m / 30000m；每一圈都會補 Google 路線時間，優先湊滿 5 張符合走路 15 分鐘內或開車 30 分鐘內的卡片。若最大圈仍不足 5 張，才用同圈內最接近的路線候選補足，但不放寬關鍵字、營業、吃法、飲食等強條件。
...
- 走路 / 開車模式會用定位點做同心圓候選搜尋，先以 Google 路線時間過濾，再依綜合分數與路線結果顯示前 5 張。
- 詳情仍保留步行與開車兩種估算。
