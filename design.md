...
這是一個手機直式使用的美食排行榜 Web App。首頁直接進入「在地美食榜」，以 Google Places 真實餐廳資料為核心，依使用者目前時間、定位地區、濾網條件與綜合評分排序，快速列出前 5 名餐廳。
...
- `營業` 預設為 `營業中`。
- `交通` 預設不選；未選交通時，卡片路線仍以走路估算顯示。
- `時段` 會依使用者當下時間自動帶入。
- `地區` 會嘗試依使用者定位反查縣市、區域、里。
- `地區` 與 `交通` 是互斥濾網：選地區會清掉交通，選走路或開車會清掉縣市 / 區域 / 里。
