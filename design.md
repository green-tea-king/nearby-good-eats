...
1. 頁首
   ...
   - 右側只保留濾網按鈕，移除評分說明與重新整理，避免手機頁首按鈕過多。
...
6. 吃法：單點 / 吃到飽
7. 飲食：葷食 / 素食
8. 評鑑：米其林星 / 米其林入選 / 必比登 / 綠星 / 500盤 / 500碗
...
- `評鑑` 是本地批次資料硬濾網，只讀 `assets/awards-taiwan.json` 已整理的米其林、必比登、綠星、500盤與 500碗資料；不把評鑑選項丟進 Google 搜尋詞，也不增加即時 Google API 調用。
...
- 評論可信度保守扣分只處理「高星等但樣本數偏少」的情況，例如 4.9 星但未滿 200 則評論會小幅扣分；這不是判定洗評，而是避免少量五星或活動換評過度影響排行。詳情內會顯示評論樣本說明。
